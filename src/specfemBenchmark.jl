...
Assemble SPECFEM2D's genuine `forward_image*.jpg` wavefield snapshots into an
...
function make_specfem2d_snapshot_video(
    ...
    images = sort(filter(path -> occursin(
        r"^forward_image[0-9]+\.jpg$", basename(path)),
        readdir(directory; join=true)))
    isempty(images) && throw(ArgumentError(
        "no SPECFEM2D forward_image*.jpg snapshots found in $directory",
    ))
    ...
    pattern = joinpath(directory, "forward_image*.jpg")
